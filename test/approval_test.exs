defmodule ApprovalTest do
  ...
  test "creates a reference file if it doesn't exist" do
    ...
    clean_dir("test/support/sandbox1")
    ...
    try do
      ...
      capture_log(fn ->
        ...
        approve snapshot: File.read!("test/support/image-test-reference.png"),
                reference: File.read!("test/support/sandbox1/image-test-reference.png"),
                reviewed: false

        assert File.exists?("test/support/sandbox1/image-test-reference.png")
      end)
    after
  ...
  test "an approval test with a reference that doesn't exist raises a warning" do
    ...
    try do
      ...
      assert log_message =~ "must be approved"
      assert log_message =~ "test/support/sandbox1/image-test-reference.png"
    after
      # Make sure we clean up the directory, no matter what happens
      # (it's cleaner that way)
      clean_dir("test/support/sandbox1")
  ...
  test "when the test has not been reviewed, it always fails (case 1: snapshot doesn't match reference)" do
    assert_raise(Approval.ApprovalError, fn ->
      capture_log(fn ->
        approve snapshot: File.read!("test/support/image-test-wrong.png"),
                reference: File.read!("test/support/image-test-reference.png"),
                reviewed: false
      end)
    end)
  ...
  test "when the test has been reviewed and the snapshot matches the reference, the test fails" do
    assert_raise(Approval.ApprovalError, fn ->
      approve snapshot: File.read!("test/support/image-wrong-test.png"),
              reference: File.read!("test/support/image-test-reference.png"),
              reviewed: true
    end)
  end
  ...
  test "if the snapshot doesn't match the reference, a *.diff.html file is generated" do
    clean_dir("test/support/sandbox2")

    try do
      File.cp!(
        "test/support/image-test-reference.png",
        "test/support/sandbox2/image-test-reference.png"
      )

      # The following will raise, but we will catch the exception and continue
      assert_raise(Approval.ApprovalError, fn ->
        approve snapshot: File.read!("test/support/image-test-wrong.png"),
                reference: File.read!("test/support/sandbox2/image-test-reference.png"),
                reviewed: true
      end)

      assert File.exists?("test/support/sandbox2/image-test-reference.png" <> ".diff.html")
    after
      clean_dir("test/support/sandbox2")
    end
  end
  ...
  test "The generated *.diff.html file contains the right HTML, CSS and JS" do
    ...
    try do
      ...
      diff_file = "test/support/sandbox2/image-test-reference.png" <> ".diff.html"
